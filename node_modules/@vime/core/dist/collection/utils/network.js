 ...
 */
export const isObjOrJSON = (input) => !isNil(input) &&
  (isObject(input) || (isString(input) && input.startsWith('{')));
/**
 * If an object return otherwise try to parse it as json.
 */
export const objOrParseJSON = (input) => isObject(input) ? input : tryParseJSON(input);
/**
...
export const loadScript = (src, onLoad, onError = noop) => {
  var _a;
 ...
 */
export const appendParamsToURL = (url, params) => appendQueryStringToURL(url, isObject(params)
