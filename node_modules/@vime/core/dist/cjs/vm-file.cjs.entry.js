 ...
 */
class VideoFullscreenController extends FullscreenController.FullscreenController {
...
class VideoPresentationController {
  constructor(host) {
    ...
    this.emitter = FullscreenController.mitt();
  ...
  get isSupported() {
    var _a, _b, _c;
    return (support.IS_IOS &&
      withComponentRegistry.isFunction((_a = this.videoElement) === null || _a === void 0 ? void 0 : _a.webkitSetPresentationMode) &&
  ...
  addPresentationModeChangeEventListener() {
    if (!this.isSupported || withComponentRegistry.isNil(this.videoElement))
      return withComponentRegistry.noop;
    return withComponentRegistry.listen(this.videoElement, 'webkitpresentationmodechanged', this.handlePresentationModeChange.bind(this));
  }
  handlePresentationModeChange() {
...
const File = class {
  constructor(hostRef) {
    ...
    this.vmMediaElChange = index.createEvent(this, "vmMediaElChange", 7);
    this.vmSrcSetChange = index.createEvent(this, "vmSrcSetChange", 7);
    this.textTracksDisposal = new withComponentRegistry.Disposal();
    this.wasPausedBeforeSeeking = true;
    this.currentSrcSet = [];
    this.mediaQueryDisposal = new withComponentRegistry.Disposal();
    ...
    withComponentRegistry.watchComponentRegistry(this, 'vm-poster', regs => {
      [this.vmPoster] = regs;
    });
  }
  onMediaTitleChange() {
  ...
  initLazyLoader() {
    this.lazyLoader = new LazyLoader.LazyLoader(this.host, ['data-src', 'data-poster'], () => {
      if (withComponentRegistry.isNil(this.mediaEl))
        return;
      const poster = this.mediaEl.getAttribute('data-poster');
      if (!withComponentRegistry.isNull(poster))
  ...
  hasCustomPoster() {
    return !support.IS_IOS && !withComponentRegistry.isUndefined(this.vmPoster);
  }
  cancelTimeUpdates() {
    if (withComponentRegistry.isNumber(this.timeRAF))
  ...
  getAdapter() {
    return __awaiter(this, void 0, void 0, function* () {
      return {
        ...
        canPlay: (type) => __awaiter(this, void 0, void 0, function* () { return withComponentRegistry.isString(type) && (utils.audioRegex.test(type) || utils.videoRegex.test(type)); }),
        ...
        canSetPiP: () => __awaiter(this, void 0, void 0, function* () { return support.canUsePiP(); }),
  ...
  listenToTextTracksForChanges() {
    if (this.hasCustomTextManager)
      return;
    this.textTracksDisposal.empty();
    if (withComponentRegistry.isUndefined(this.mediaEl))
      return;
  ...
  render() {
    ...
    return this.viewType === ViewType.ViewType.Audio ? audio : video;
  }
